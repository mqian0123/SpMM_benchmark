...
Csc<T,ITYPE>::Csc(Triple<T, ITYPE> * triples, ITYPE size, ITYPE rows, ITYPE cols, bool isSym)
    : nz(size), m(rows), n(cols), issym(false) // always treat as general
{
    assert(size != 0 && n != 0);

    num = new T[nz];         // nonzero values
    ir  = new ITYPE[nz];     // row indices
    jc  = new ITYPE[n + 1];  // column pointers

    ITYPE *w = new ITYPE[n]; // workspace for counts / write pointers

    // 1) Count number of entries per column
    for (ITYPE k = 0; k < n; ++k)
        w[k] = 0;
    for (ITYPE k = 0; k < nz; ++k)
        w[triples[k].col]++;

    // 2) Compute cumulative sum (convert counts → starting indices)
    ITYPE cumsum = 0;
    for (ITYPE col = 0; col < n; ++col)
    {
        ITYPE temp = w[col];  // store original count
        w[col] = cumsum;      // starting index for this column
        jc[col] = cumsum;     // also save in jc
        cumsum += temp;
    }
    jc[n] = cumsum; // total number of nonzeros

    assert(cumsum == nz);

    // 3) Fill ir and num arrays
    for (ITYPE k = 0; k < nz; ++k)
    {
        ITYPE col = triples[k].col;
        ITYPE dest = w[col]++; // get position and advance write pointer
        ir[dest] = triples[k].row;
        num[dest] = triples[k].val;
    }
